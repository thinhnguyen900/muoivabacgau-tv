...
for(let r=0;r<30;r++){
 for(const [cat,text,perf,lang] of corpus){
  ...
  if(prev&&prev.performance!==plan.performance)ok(cat+'-state-diff-'+r,[plan.gaze,plan.head,plan.brow,plan.mouth,plan.gesture].some((v,i)=>v!==[prev.gaze,prev.head,prev.brow,prev.mouth,prev.gesture][i]),{prev,plan});
  prev=plan;
 }
}
...
console.log(JSON.stringify({total:pass+fail.length,pass,fail:fail.length,conversationTurns:corpus.length*30,states:['welcoming','listening','thinking','gentle','playful'],audit},null,2));
