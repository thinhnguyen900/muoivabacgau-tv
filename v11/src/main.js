...
function makeBackdropTexture(){
  ...
  const g=x.createLinearGradient(0,0,0,1024);g.addColorStop(0,'#799f9b');g.addColorStop(.48,'#8eb19d');g.addColorStop(1,'#c3b58c');x.fillStyle=g;x.fillRect(0,0,1024,1024);
  const glows=[[180,250,190,'rgba(255,238,190,.18)'],[790,200,240,'rgba(228,246,221,.12)'],[700,600,180,'rgba(255,226,170,.10)'],[320,710,230,'rgba(79,110,78,.10)']];
  ...
  x.globalAlpha=.10;x.fillStyle='#365d4e';for(let i=0;i<16;i++){const px=(i*197)%1120-50,py=680+(i*83)%250,rr=90+(i*29)%120;x.beginPath();x.arc(px,py,rr,0,Math.PI*2);x.fill();}x.globalAlpha=1;
  const tex=new THREE.CanvasTexture(c);tex.colorSpace=THREE.SRGBColorSpace;tex.minFilter=THREE.LinearFilter;return tex;
}

const scene=new THREE.Scene();scene.background=new THREE.Color(0x87a9a1);scene.fog=new THREE.FogExp2(0x91aa9c,0.028);
const camera=new THREE.PerspectiveCamera(30,1,.1,100);camera.position.set(0,2.94,9.15);camera.lookAt(0,2.30,.02);
const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio||1,2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=.98;stage.prepend(renderer.domElement);
...
const hemi=new THREE.HemisphereLight(0xffedd5,0x294b3b,1.18);scene.add(hemi);
const key=new THREE.DirectionalLight(0xffd7ae,2.75);key.position.set(-4.3,7.0,5.8);key.castShadow=true;key.shadow.mapSize.set(2048,2048);key.shadow.camera.near=.1;key.shadow.camera.far=24;key.shadow.camera.left=-5;key.shadow.camera.right=5;key.shadow.camera.top=7;key.shadow.camera.bottom=-2;key.shadow.bias=-0.00018;key.shadow.normalBias=0.028;scene.add(key);
const fill=new THREE.DirectionalLight(0xfff0dc,.58);fill.position.set(4.7,3.2,6.4);scene.add(fill);
const rim=new THREE.DirectionalLight(0xb8d7c9,.68);rim.position.set(5.0,5.3,-4.2);scene.add(rim);
const faceBounce=new THREE.PointLight(0xffe4c4,.42,9,2);faceBounce.position.set(-.15,3.55,4.6);scene.add(faceBounce);
const floor=new THREE.Mesh(new THREE.CircleGeometry(6.8,96),new THREE.MeshStandardMaterial({color:0x6e5a43,roughness:.97,metalness:0}));floor.rotation.x=-Math.PI/2;floor.receiveShadow=true;scene.add(floor);
const contact=new THREE.Mesh(new THREE.CircleGeometry(2.15,64),new THREE.MeshBasicMaterial({color:0x263126,transparent:true,opacity:.14,depthWrite:false}));contact.rotation.x=-Math.PI/2;contact.position.y=.006;contact.scale.set(1,.56,1);scene.add(contact);
...
debug.textContent=JSON.stringify(report,null,2);window.__BACGAU_V11__={ready:true,model:modelMode==='glb',mode:modelMode,report,visualPipeline:'authored-warm-v4-cinematic'};
