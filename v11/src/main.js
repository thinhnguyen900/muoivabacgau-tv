...
const scene=new THREE.Scene();scene.background=new THREE.Color(0x8fc8e7);scene.fog=new THREE.FogExp2(0x8fc8e7,0.035);
const camera=new THREE.PerspectiveCamera(31,1,.1,100);camera.position.set(0,2.92,9.35);camera.lookAt(0,2.28,0);
const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio||1,2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.03;stage.prepend(renderer.domElement);
...
debug.textContent=JSON.stringify(report,null,2);window.__BACGAU_V11__={ready:true,model:modelMode==='glb',mode:modelMode,report,visualPipeline:'authored-warm-v3'};
